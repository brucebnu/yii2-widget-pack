...
class LteBox extends Widget
{
    ...
    public $topTemplate
        = <<<HTML
<div {options}>
<div {headerOptions}><h3 class="card-title">{title}</h3>{box-tools}</div>
<div class="card-body">
HTML;
...
<div class="card-footer {footerOptions}">{footer}</div>
</div>
HTML;
    ...
    protected function prepareBoxTools()
    {
        ...
        return $boxTools ? Html::tag('div', $boxTools, ['class' => 'card-tools pull-right']) : '';
    }
}
